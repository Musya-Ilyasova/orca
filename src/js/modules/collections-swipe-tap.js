...
/**
  ...
  */
var swipe = function (el, settings) { // настройки по умолчанию
  ...
  var getSupportedEvents = function () {
    switch (true) {
      ...
      case support.touch:
        events = {
          type: "touch", start: "touchstart", move: "touchmove", end: "touchend", cancel: "touchcancel"
        };
        break;
      default:
        events = {
  ...
  };// добавление поддерживаемых событийvar events = getSupportedEvents();// проверка наличия мышиif ((support.pointer && !support.touch) || events.type === "mouse")
  isMouse = true;// добавление обработчиков на элементel.addEventListener(events.start, checkStart);el.addEventListener(events.move, checkMove);el.addEventListener(events.end, checkEnd);if (support.pointer && support.touch) {
  el.addEventListener('lostpointercapture', checkEnd);
};/**
  * Примеры работы swipe()
  */var getExampleDiv = function (id) { return document.getElementById("example-" + id); }; var makeDone = function (el, currentDir, dirs) {
...
}; for (var example in examples) { var el = examples[example].el; swipe(el, examples[example].set); el.addEventListener("swipe", examples[example].callback); }// let top = document.querySelector('.top');// let bottom = document.querySelector('.bottom');
